...
class MessageService(CRUDRepository):
    ...
    def list_messages(self) -> list[dict]:
        return [
            ...
            for message in self.get_many(self.session)
        ]

    def list_room_messages(self, room_id: UUID) -> list[Message]:
        statement = select(Message).where(Message.room_id == room_id).order_by(Message.created_at.asc())
        return list(self.session.exec(statement))

    def create_transcript_message(
        self, room_id: UUID, user_id: UUID | None, content: str
    ) -> Message:
        message = Message(
            room_id=room_id,
            user_id=user_id,
