...
from app.ai.query import stream_agent_response
...
def stream_ai_response(
    ...
    try:
        response_text = asyncio.run(stream_to_room(room_id, stream_agent_response(query)))
    ...
    with Session(engine) as db:
        ...
        for room in rooms:
            ...
            enqueue_ai_job(
                room.id,
                "heartbeat",
                f"The room topic is {room.topic or room.name}. Ask one concise, warm English question to restart the conversation.",
            )
            mark_room_activity(room.id)
            queued_count += 1
