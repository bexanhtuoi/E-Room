...
**Ghi chú triển khai:**
- Pipeline: Âm thanh → WebSocket → FunASR STT → Celery → LLM → WebSocket → ChatWindow
...
**Phụ thuộc:** FunASR, LLM (GPT-4o), [[ERoom/notes#WebSocket|WebSocket]], [[#F-BILL-01|Subscription]]
...
- TTS engine: Supertonic (local ONNX model)
...
**Phụ thuộc:** Supertonic, Minio, [[#F-BILL-01|Subscription]]
