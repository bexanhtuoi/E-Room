...
## ADR-002: FunASR cho Chuyển Giọng nói thành Văn bản
...
**Chuyển sang FunASR local** (SenseVoiceSmall, CPU) thay vì OpenAI Whisper API. Lý do: giảm chi phí, chạy offline, tích hợp sâu với Wav2Vec2 alignment.

### Hệ quả
- **FunASR SenseVoiceSmall** chạy local trên CPU, real-time factor < 1
- Không cần kết nối internet cho STT
- **Tích hợp trong PronunciationPipeline**: FunASR + Wav2Vec2 + CMU Dictionary
...
model = AutoModel("iic/speech_seaco_paraformer_large_speech_encoder_asr_nat-zh-cn-16k-common-vocab8404-pytorch")
```

### Phương án đã cân nhắc
| Phương án | Lý do từ chối |
|-------------|-------------|
| OpenAI Whisper API | Tốn chi phí, phụ thuộc internet (đã thay bằng FunASR) |
...
- Tích hợp AI/ML tự nhiên (FunASR, LLM, SentenceTransformer)
...
## ADR-006: Local LLM (llama.cpp) cho Sửa lỗi/Đánh giá/Nhịp tim

**Trạng thái:** ✅ Đã chấp nhận (Đã thay đổi) | **Ngày:** 2026-05-04 | **Cập nhật:** 2026-06-06

### Bối cảnh
Năm use case LLM riêng biệt: (1) Sửa lỗi — nhanh, JSON có cấu trúc, (2) Đánh giá — đầu vào dài, (3) Nhịp tim — rất nhanh, độ phức tạp thấp, (4) Expert RAG Q&A, (5) Intent classifier chống lạm dụng.

### Quyết định (Đã thay đổi — 2026-06-06)
**Đã chuyển từ LM Studio sang llama.cpp** (OpenAI-compatible server). Hai model chạy trên 2 cổng riêng:
- **Text gen**: `gemma-4-E2B-it-Q8_0.gguf` tại `http://localhost:8012/v1` (api_key: `dev`)
- **Embedding**: `Qwen3-Embedding-0.6B-Q8_0.gguf` tại `http://localhost:8013/v1` (api_key: `dev`)
Lý do: llama.cpp nhẹ hơn, không cần GUI, kiểm soát tốt hơn qua CLI, và tách biệt text gen với embedding.

### Hệ quả
- **Chi phí $0** cho LLM inference (chạy local)
- **OpenAI-compatible API**: Dùng `ChatOpenAI` từ langchain-openai với `base_url` + `api_key` trỏ đến llama.cpp server
- **2 model riêng**: Gemma 4 E2B cho chat/text gen, Qwen3 Embedding cho vector embedding
...
llm_api_key: str = "dev"
embedding_base_url: str = "http://localhost:8013/v1"
...
### Quyết định
**Supertonic ONNX model** (local CPU) cho Giai đoạn 3.

### Hệ quả

**Tích cực:**
- Tích hợp đơn giản — cùng API key với LLM
...
**Tiêu cực:**
- Tiếng Việt phát âm chưa hoàn hảo (Supertonic đang cải thiện)
...
| Phương án | Lý do từ chối |
|-------------|-------------|
| Supertonic | Local ONNX, free, chất lượng tốt cho en + vi |
...
## ADR-018: Pronunciation Pipeline — FunASR + Wav2Vec2 + CMU Dictionary
...
1. **FunASR SenseVoiceSmall** (CPU) — ASR → transcript + word confidence
...
- Graceful degradation: FunASR fail → Wav2Vec2 vẫn chạy, và ngược lại
- Tích hợp trong pipeline `ws/speech.py:process_speech()` — synchronous nhưng non-blocking (ThreadPoolExecutor)

**Tiêu cực:**
- Cần tải 2 model lần đầu: FunASR SenseVoiceSmall + Wav2Vec2
...
### Files triển khai
- `infrastructure/audio_funasr.py` — FunASR AutoModel singleton
...
timeline
    ...
    2026-05-04 : ADR-001 LiveKit
               : ADR-002 FunASR (API→Local)
               : ADR-003 MySQL+Redis
               : ADR-004 FastAPI
               : ADR-005 React+Vite
               : ADR-006 GPT-4o→Local LLM (LM Studio→llama.cpp 06-06)
               : ADR-007 Nhóm Nhỏ
               : ADR-008 Celery
               : ADR-009 JWT Auth
               : ADR-010 uv
